import { login, getInfo, loginMap } from "@/api/login"
...
const user = {
  ...
  actions: {
    ...
    LoginLed ({ commit }, role) {
      loginMap({identity: role}).then(res => {
        if (res.result.token) {
          setToken(res.result.token, false, "LED")
          commit("SET_LED_TOKEN", res.result.token)
        }
      }).catch(error => {
        console.error(error)
      })
