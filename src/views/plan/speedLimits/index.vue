<template>
  <div>
    ...
    <el-table border ref="multipleTable" v-loading="loading" v-show="!loading" :data="datalist" tooltip-effect="dark"
      ...
      <el-table-column prop="plannedEventType" label="降速措施类型" style="width: 30%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="speedLimitsName" label="限速措施名称" style="width: 30%"></el-table-column>
      <el-table-column prop="speedLimitsDes" label="限速措施描述" style="width: 20%"></el-table-column>
...
import { doDel } from '@/api/speedLimits'
...
export default {
  ...
  methods: {
    beforeInit () {
      this.url = 'plan/getSpeedLimitsList'
      const sort = 'Id asc'
      this.params = {
    ...
    showEdit (val) {
      ...
      this.$refs.form.dataForm = { id: val.id,
        ...
        speedLimitsName: val.speedLimitsName,
        speedLimitsDes: val.speedLimitsDes,
        speedLimitsValue: val.speedLimitsValue }
    },
    changeOptions () {
      getPlannedEventTypes('speed').then(res => {
