<template>
  <div>
    ...
    <el-table border ref="multipleTable" v-loading="loading" v-show="!loading" :data="datalist" tooltip-effect="dark"
      ...
      <el-table-column prop="eventName" label="预案事件类型名称" style="width: 30%"></el-table-column>
      <el-table-column prop="plannedEventCode" label="预案事件类型标识" style="width: 30%"></el-table-column>
      <el-table-column prop="plannedEventType" label="预案事件类型类别" style="width: 30%"></el-table-column>
...
import { doDel, getEventTyps } from '@/api/plannedEventType'
...
export default {
  ...
  methods: {
    ...
    showEdit (val) {
      ...
      this.$refs.form.dataForm = { id: val.id, eventCode: val.plannedEventCode, plannedEventType: val.plannedEventType, sort: val.sort }
    },
    changeOptions () {
      getEventTyps().then(res => {
