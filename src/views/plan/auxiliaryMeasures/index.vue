<template>
  <div>
    ...
    <el-table border ref="multipleTable" v-loading="loading" v-show="!loading" :data="datalist" tooltip-effect="dark"
      ...
      <el-table-column prop="plannedEventType" label="辅助措施类型" style="width: 30%" show-overflow-tooltip></el-table-column>
      <el-table-column prop="auxiliaryMeasuresName" label="辅助措施名称" style="width: 30%"></el-table-column>
      <el-table-column prop="auxiliaryMeasuresDes" label="辅助措施描述" style="width: 20%"></el-table-column>
...
import { doDel } from '@/api/auxiliaryMeasures'
...
export default {
  ...
  methods: {
    beforeInit () {
      this.url = 'plan/getAuxiliaryMeasuresList'
      const sort = 'Id asc'
      this.params = {
    ...
    showEdit (val) {
      ...
      this.$refs.form.dataForm = { id: val.id, plannedEventTypeIds: val.plannedEventTypeIds, auxiliaryMeasuresName: val.auxiliaryMeasuresName, auxiliaryMeasuresDes: val.auxiliaryMeasuresDes }
    },
    changeOptions () {
      getPlannedEventTypes('auxiliary').then(res => {
