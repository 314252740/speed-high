  ...
  <el-dialog :title="title" :closeOnClickModal="false" :visible.sync="visible" width="550px">
    <el-form ref="form" :model="dataForm" :rules="rules" size="small" label-position="right" label-width="130px" style="padding: 0;margin: 0">
      <el-form-item label="父节点名称" v-show="showparent">
        <el-input v-model="dataForm.resourceName" required="true" disabled="disabled" autocomplete="off" ></el-input>
      </el-form-item>
      <el-form-item label="名称" prop="name">
        <el-input v-model="dataForm.name"  autocomplete="off" minlength="2" maxlength="20"></el-input>
      </el-form-item>
      <el-form-item label="编码" prop="code">
        <el-input v-model="dataForm.code"  autocomplete="off" ></el-input>
      </el-form-item>
      ...
      <el-form-item label="组件路径" v-show="(!compdisabled && dataForm.leaf)" prop="component" :rules="{required:(!compdisabled && dataForm.leaf), message: '组件路径不能为空', trigger: 'blur'}">
        <el-input type="text" v-model="dataForm.component" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="访问地址(path)"  v-show="!btnshow" prop="content" :rules="{required:!btnshow, message: '访问地址不能为空', trigger: 'blur'}">
        <el-input type="text"  v-model="dataForm.content" autocomplete="off"></el-input>
      </el-form-item>
    </el-form>
...
<script>
import { doAddMenu, doAddModule, edit } from '@/api/menu'
export default {
  data () {
    return {
      loading: false,
      ...
      title: '添加模块',
      dataForm: {
        id: '', name: '', parentid: '', resourceName: '', type: '0', code: '', sortOrder: '', icon: '', component: '', content: '', iframe: '0', leaf: false
      },
      rules: {
        name: [
        ...
        if (valid) {
          ...
          if (this.isAdd) {
            ...
            if (this.compdisabled) {
              ...
              doAddModule(this.dataForm).then(res => {
                debugger
                if (res.result === 'ok') {
                  this.resetForm()
                  this.$notify({
              ...
              doAddMenu(this.dataForm).then(res => {
                this.resetForm()
                this.$notify({
                  title: '添加成功',
                  type: 'success',
