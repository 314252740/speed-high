    ...
    <el-col :span="24">
      <div style="margin-bottom:10px">
        <el-button-group>
          ...
          <el-button type="warning" icon="el-icon-share" @click="grantResource()" v-has>资源授权</el-button>
        </el-button-group>
      </div>
    </el-col>
...
import resDialog from './module/res-dialog'

export default {
  mixins: [initData],
  components: {
    search, eform, resDialog
  },
  data () {
    return {
      list: [],
  ...
  methods: {
    ...
    showEdit (val) {
      ...
      this.$refs.form.dataForm = {
        id: val.id, name: val.name, code: val.code, memo: val.memo }
    },
    removeById (id) {
    ...
    grantResource () {
      ...
      this.$refs.res.init(ids.join(','))
      this.$refs.res.roleIds = ids.join(',')
      this.$refs.res.resvisible = true
      this.$refs.res.rolenames = '待授权的角色为：' + names.join(',')
    }
  },
  computed: {}
}
