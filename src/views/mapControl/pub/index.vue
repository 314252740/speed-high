...
<template>
  <div>
    ...
    <el-table border ref="multipleTable" v-loading="loading" v-show="!loading" :data="datalist" tooltip-effect="dark"
      ...
      <el-table-column prop="name" label="枢纽名称" width="150px"></el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
            <span v-if="scope.row.canRound === '0'">无分流能力</span>
            <span v-if="scope.row.canRound === '1'">一级分流能力</span>
            <span v-if="scope.row.canRound === '2'">二级流能力</span>
            <span v-if="scope.row.canRound === '3'">三级流能力</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    showEdit (val) {
      ...
      this.$refs.form.dataForm = {
        ...
        roadNamet: val.roadNamet,
        roadCodet: val.roadCodet,
        status: val.status,
        pileId: val.pileId,
        canRound: Number(val.canRound),
        roadId: val.roadId,
        roadtId: val.roadtId,
        pileInfo: val.pileInfo,
        piletInfo: val.piletInfo }
      this.$refs.form.startDraw()
    },
    removeById (id) {
