<template>
  <div class="varAnalysis" id="varAnalysis" ref="varAnalysis">
    ...
    <myDialog ref="myDialog" :list="list"></myDialog>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      idx: -1,
      navlist: [
        { name: '路段平均速度统计', enName: 'getAverageSpeedCar' },
        { name: '低速路段统计', enName: 'getLowSpeedCar' },
        { name: '超速路段统计', enName: 'getHighSpeedCar' }
      ],
      infoList: [],
      infoType: {
        getAverageSpeedCar: { number: { name: '路段平均速度', value: null, unit: 'km/h' } },
        getLowSpeedCar: { number: { name: '低速路段速度', value: null, unit: 'km/h' } },
        getHighSpeedCar: { number: { name: '超速路段速度', value: null, unit: 'km/h' } }
      },
      setTree: [],
      defaultProps: {
        children: 'children',
  ...
  methods: {
    ...
    async chooseItem (index, enName) {
      ...
      this.idx = index
      this.url = enName
      if (this.url === 'getAverageSpeedCar') {
