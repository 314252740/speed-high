...
<script>
import { doDelete } from '@/api/highnumbercontent'
...
export default {
  ...
  methods: {
    beforeInit () {
      this.url = 'module/highnumbercontents'
      const sort = 'id desc'
      this.params = {
        page: this.page + 1,
        sortType: sort,
    ...
    showEdit (val) {
      ...
      this.$refs.form.dataForm = {
        id: val.id, type: val.type, reviewer: val.reviewer, status: val.status, enters: val.enters, tHighId: val.tHighId, content: val.content }
      this.$refs.form.startDraw()
    },
    removeById (id) {
