<template>
  <el-dialog :title="isAdd ? '新增诱导屏信息' : '修改诱导屏信息'" :closeOnClickModal="false" :visible.sync="visible" width="1200px" label-width="80px" @open="changeHeight" @close="hide">
    <div class="container">
      ...
      <div class="mapCont"  style="width:40%" ref=dialogForm>
        <el-form ref="form" :model="dataForm" :rules="rules" size="small" label-position="right" label-width="70px" >
          ...
          <el-row>
            <el-select style="width:50%;" clearable v-model="dataForm.roadId" placeholder="请选择道路" required="true" autocomplete="off" @change="rcod(dataForm.roadId)">
              <el-option
          ...
          <el-form-item label="摄像机ID" prop="cameraId" label-width="77px">
            <el-input clearable style="width:98%" v-model="dataForm.cameraId" required="true" minlength="6" maxlength="20"  autocomplete="off" ></el-input>
          </el-form-item>
          <el-form-item label="尺寸" prop="size">
            <el-input clearable v-model="dataForm.size" required="true" minlength="6" maxlength="20"  autocomplete="off" ></el-input>
          </el-form-item>
          <el-form-item label="备注" prop="note">
            <el-input clearable v-model="dataForm.note" required="true" minlength="6" maxlength="2000"  autocomplete="off" ></el-input>
          </el-form-item>
          ...
          <el-form-item label="工作方式" prop="workWay" label-width="98px">
            <el-radio v-model="dataForm.workWay" :label="0">自动</el-radio>
            <el-radio v-model="dataForm.workWay" :label="1">手动</el-radio>
          </el-form-item>

          <el-form-item label="网络状态" prop="networkStatus" label-width='98px'>
...
import {doAdd, doUpdate} from "@/api/screen"
// import request from "@/utils/request"
import tileMap from "@/components/map/tileMap";// 初始化瓦片地图
import {getRoadinfo} from "@/api/road"
...
export default {
  ...
  data () {
    return {
      ...
      visible: false,
      isAdd: false,
      dataForm: {id: "", ip: "", name: "", roadName: "", roadCode: "", longitude: "", latitude: "", workWay: "", networkStatus: "", size: "", 
      manufacturer: "", cameraId: "", note: "", roadId: "",cameraIps:""},
      rules: {
        name: [
  ...
  methods: {
    ...
    rcod(roadId) {
        getRoadinfo(roadId)
            .then(res => {
              this.dataForm.roadCode = res.data.code;
              this.dataForm.roadName = res.data.name;
            })
    ...
    startDraw() {
      ...
      //     });
      this.$nextTick(()=>{
        this.$refs.map.startDrawPoint("screenInduction", this.setLnglat, [this.dataForm.longitude, this.dataForm.latitude]);
      })
    },
    setLnglat(point) {
    ...
    resetForm () {
      ...
      this.dataForm = {id: "", ip: "", name: "", roadName: "", roadCode: "", longitude: "", latitude: "", workWay: "", networkStatus: "",size: "", manufacturer: "", cameraId: "", note: "", roadId: "",cameraIds:"",sign:""}
    },
    doSubmit () {
